...
def listar_e_salvar_participantes(email_logado, titulo_evento, eventos):
    if titulo_evento in eventos:
        evento = eventos[titulo_evento]
        if evento['criador'] == email_logado:
            nome_arquivo = f"{titulo_evento}_participantes.txt"
            arquivo = open(nome_arquivo, "w")
            arquivo.write(f"Participantes do evento {titulo_evento}:\n")
            for participante in evento['participantes']:
                arquivo.write(f"- {participante}\n")
            arquivo.close()
            print(f"Lista de participantes salva em {nome_arquivo}")
        else:
